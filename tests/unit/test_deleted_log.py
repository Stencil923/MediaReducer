"""deleted.log parsing: every line generation must parse, size_bytes must
survive the rationale fields, and the history line carries the why."""
import sys
...
sample = [
    "2026-07-14 10:00:00 | Old Movie | /library/movies/O/O.mkv | size_bytes=4000000000",
...
check("old line has no why", "score" not in entries[0]["line"])
check("new line shows why", "score 7.5" in entries[1]["line"] and "3 plays" in entries[1]["line"]
...
check("reclaimed bytes sum all generations", stats["reclaimed_bytes"] == 4000000000 + 5000000000 + 123)
