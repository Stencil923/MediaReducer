...
| `test_startup_invalidation` | Startup PRESERVES the store — a recent, unchanged plan stays usable with no re-scan. Whether it can still be trusted is decided live: a full scan older than two days (`_full_scan_overdue`), a monitored-path change, or a plan-affecting config change locks Cleanup + arming until a fresh Simulate; protected-collection changes do not. A completed scan lifts the lock. Also covers the once-a-day Monitor-Only maintenance Simulate (`_paused_daily_scan_due`) and the fresh-install onboarding case |
...
| `test_reconcile_from_snapshot` | The config-save **reconcile** (`reconcile_from_snapshot`): rebuild the marked & eligible queue from the stored snapshot under new filter/scoring/threshold config with NO library walk and NO server fetch — eligibility recomputes both directions (turning a filter off RE-ADMITS movies), each still-marked movie keeps its delay clock while a newly-marked one gets a fresh one and a now-ineligible one drops off, the marked prefix tracks the space target, the stored `excluded` (identity-mismatch) flag is never re-admitted, and the refetch path updates protected/favorite facts from injected lookups |
| `test_reconcile_trigger` | The app-side reconcile decision (`_reconcile_after_save`): a scoring/threshold change reconciles with no server call; a protected-collection or favorites change re-fetches; a refetch change with the server down is HELD (never run against stale protection) and retried once the connection recovers or the server is disabled; unrelated or no-monitored-paths saves reconcile nothing |
